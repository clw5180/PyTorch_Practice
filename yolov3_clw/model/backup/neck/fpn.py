...
class Fpn(nn.Module):
    def __init__(self, in_channels, out_channels):  # 默认 [1024, 512, 256] 和 [(cls_num + 1 + 4)*3] *3
        ...
        self.conv1_1 = nn.Sequential(
            Convolutional(512, 1024, 3, stride=1, padding=1),
            nn.Conv2d(1024, out_channel1, 1, stride=1, padding=0)    # clw note: 这里out_channels = (cls_num + 4 + 1) * 3, cls_num就是类别数
    ...
    def forward(self, Y_74, Y_61, Y_36):
        Y_79 = self.conv1_0(Y_74)  # input1 of yolo_layer1
        Y_81 = self.conv1_1(Y_79)  # output1 of yolo_layer1, detect large object, small feature map
        ...
        Y_105 = self.conv3_2(Y_98)  # out3, from yolo_layer3, detect small object, big feature map
        return Y_81, Y_93, Y_105